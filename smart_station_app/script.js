...
function getCrowdRangeByHour(hour) {
  if (hour >= 7 && hour <= 10) return [60, 95];
  if (hour >= 11 && hour <= 16) return [30, 65];
  if (hour >= 17 && hour <= 21) return [55, 90];
  return [10, 35];
}

function generateCrowdLevel(min, max) {
  const level = Math.floor(Math.random() * (max - min + 1)) + min;

  let label = "Low";
  if (level > 70) label = "High";
  else if (level > 40) label = "Med";

  return { level, label };
}

function generateHourlyCrowd(hour) {
  const [min, max] = getCrowdRangeByHour(hour);

  return buses.map(bus => ({
    bus,
    ...generateCrowdLevel(min, max)
  }));
}
...
document.getElementById("search-form").addEventListener("submit", e => {
  ...
  const crowdData = generateHourlyCrowd(hour);

  renderCrowd(crowdData);
});

function renderCrowd(crowdData) {
  ...
  crowdData.forEach(item => {
    ...
    else if (item.level > 40) colorClass = "medium";

    bar.className = `bar ${colorClass}`;
    bar.style.height = item.level + "%";
    bar.setAttribute(
      "data-label",
      `Bus ${item.bus} • ${item.level}% (${item.label})`
    );
...
document.addEventListener("DOMContentLoaded", () => {
  renderCrowd(generateHourlyCrowd(new Date().getHours()));
});
...
//   buses.map(b => ({
//     bus: b.bus,
//     ...generateCrowdLevel(60, 95)
//   })),

//   // Midday (medium crowd)
//   buses.map(b => ({
//     bus: b.bus,
//     ...generateCrowdLevel(30, 65)
//   })),

//   // Evening rush (high crowd again)
//   buses.map(b => ({
//     bus: b.bus,
//     ...generateCrowdLevel(55, 90)
//   })),
// ];
...
//     console.log(value); // your search text
// });
...
//   const slotIndex = getTimeSlot(timeValue);
